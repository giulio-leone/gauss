// =============================================================================
// Neon Vector Store Adapter — Implements VectorStorePort
// =============================================================================
//
// Requires: @neondatabase/serverless (peer dependency)
//
// Serverless Postgres with pgvector extension.
//
// Usage:
//   const store = new NeonStoreAdapter({
...
export interface NeonStoreConfig {
...
export interface NeonStoreOptions {
  /** Pre-configured neon sql/pool client */
  client?: any;
  /** Config to create a client internally */
  config?: NeonStoreConfig;
...
export class NeonStoreAdapter implements VectorStorePort {
  private client: any;
  private readonly tableName: string;
  private readonly dimensions: number;
  private readonly batchSize: number;
  private readonly options: NeonStoreOptions;

  constructor(options: NeonStoreOptions) {
  ...
  async initialize(): Promise<void> {
    if (!this.client) {
      if (!this.options.config) {
        throw new Error("NeonStoreAdapter: either client or config.connectionString is required");
      }
      const neon = await import("@neondatabase/serverless");
      const Pool = neon.Pool ?? (neon as any).default?.Pool;
  ...
  private ensureInitialized(): void {
    if (!this.client) {
      throw new Error("NeonStoreAdapter: call initialize() before using the adapter");
    }
  }
}
